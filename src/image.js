...
// 0104-0133
    const NintendoGraphic = [0xCE, 0xED, 0x66, 0x66, 0xCC, 0x0D, 0x00, 0x0B, 0x03, 0x73, 0x00, 0x83, 0x00, 0x0C, 0x00, 0x0D,
    ...
    const RAM_SIZES = {0x00: 0, 0x01: 1, 0x02: 1, 0x03: 4, 0x04: 16};
    ...
    function load(bytes) {
        let gameTitle = ""; for (let p = 0x0134; p <= 0x0142 && bytes[p] != 0x00; p++) gameTitle += String.fromCharCode(bytes[p]);
        let colorGB = bytes[0x0143] == 0x80 || bytes[0x0143] == 0xC0;
        let superGB = bytes[0x0146] == 0x03;
        let cartType = CARTRIDGE_TYPES[bytes[0x147]] || {name: "Unknown"};
        let ROMSize = ROM_SIZES[bytes[0x0148]];
        let RAMSize = cartType.RAMSize || RAM_SIZES[bytes[0x0149]];
        let newLicense = NEW_LICENSES[(bytes[0x0144] & 0xF0) | (bytes[0x0145] & 0x0F)] || "Unknown";
        let destinationCode = DESTINATION_CODES[bytes[0x14A]] || "Unknown";
        let oldLicense = OLD_LICENSES[bytes[0x14B]] || "Unknown";
	...
	//Real Gameboy won't run if it's invalid. We are checking just to be sure that input file is Gameboy ROM
        for (let b = 0; b < NintendoGraphic.length; b++)
		    if (NintendoGraphic[b] != bytes[b + 0x0104])
                break;
	...
	//Real Gameboy won't run if it's invalid. We are checking just to be sure that input file is Gameboy ROM
        let Complement = 0;
        for (let b = 0x134; b <= 0x14C; b++) Complement = Complement - bytes[b] - 1; 
        if (Complement != bytes[0x14D]) return;
